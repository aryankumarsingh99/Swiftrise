...
export default function JourneySection() {
  ...
  const images = [
    ...
    "https://images.unsplash.com/photo-1576400883215-7083980b6193?q=80&w=1113&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D",
      ...
      <div className="mx-auto w-[96vw] max-w-330">
          ...
          <div className="order-2 md:order-1 animate-fadeInUp" style={{ animationDelay: '200ms' }}>
            ...
            <div className="mt-10 flex items-center gap-6 text-[1.7rem] text-[#ffffff]">
              ...
              <p className="text-[1.75rem] tracking-tight font-semibold">
                {String(imageIndex + 1).padStart(2, '0')}/<span className="text-[#8e93a2]">04</span>
              </p>
              <button 
                type="button"
        ...
        <div className="mt-8 animate-fadeInUp" style={{ animationDelay: '400ms' }}>
          ...
          <div className="-mt-1.75 grid grid-cols-4 items-start">
            {[
              { id: "01", active: imageIndex === 0 },
              { id: "02", active: imageIndex === 1 },
              { id: "03", active: imageIndex === 2 },
              { id: "04", active: imageIndex === 3 },
            ].map((item, idx) => (
              <div 
                key={item.id} 
              ...
              >
                <span
                  className={`mb-2 block size-3 rounded-full transition-all duration-300 mx-auto ${
                    item.active ? "bg-[#ffffff] shadow-lg shadow-white/50 animate-pulse-slow scale-125" : "bg-[#838383] group-hover:scale-150 group-hover:shadow-md group-hover:shadow-[#838383]/50"
                  }`}
                  aria-hidden="true"
                />
                <p className={`text-[1.65rem] tracking-tight transition-all duration-300 font-semibold ${item.active ? "text-[#ffffff] font-bold" : "text-[#8e93a2] group-hover:text-[#ffffff]"}`}>{item.id}</p>
              </div>
            ))}
          </div>
        </div>
